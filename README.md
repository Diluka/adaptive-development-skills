...
### 方法 · 调查与验证（execution 方法名录内的公开方法）

公开标准方法（系统化调试、基于属性的测试、消费者驱动契约测试等）不设独立技能入口，在 `execution` 的方法名录中按完整名字提及，AI 按需选用：

| 方法 | 用途 |
|---|---|
| 系统理解（System Understanding） | 理解现有系统的结构、运行路径、历史演进与架构决策 |
| 契约核验（Contract Verification） | 核验确切版本下已安装或运行的 SDK、协议、框架与外部边界事实 |
| 系统化调试（Systematic Debugging） | 从已观察症状追踪到第一个错误状态和因果链 |
| 未知探索（Unknown Exploration） | 以最小可丢弃试验或受控探索会话回答技术未知与行为风险 |
