...
1. **明确边界与缺口**：确认工作单元、当前节点、目标、范围、授权与待拒绝的合理错误。每项拟议改动对应明确需求或使其成立的必要依赖；分别核实问题是否成立、修复是否属于本次范围。判断必要依赖时问：保持此处不变，能否正确实现、验证和交付当前任务？可以则作为范围外发现报告。盘点当前上下文（包括同一会话）已有结论、证据与适用条件；已有事实足以回答就直接执行或回答，不为动作词建立方法循环。
...
4. **反馈、重选或转换**：用户纠正需求理解或否定方案时，从任务原始基线复查完整差异，修正或撤回依赖旧假设的实现、测试与文档，再落实当前方向。结果只改变当前动作、方法或证据组合时，在原节点内修正后继续；改变工作单元、级别、节点、范围、授权、交付结果或副作用时，返回工作流重新编排。即将声称完成时使用独立的 [verification-before-completion](../verification-before-completion/SKILL.md) 核对最终声明。
