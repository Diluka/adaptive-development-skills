...
Workflow 的直接作业判据成立时直接执行，多步骤最多维护简短执行清单。在作业节点内处理保持用户可见行为、公共契约、架构边界和风险的普通实现取舍，并采用最简单方案。稳定概念、职责归属、生命周期、状态权威或运行接入方式仍需确定时，相关事实不足则返回工作流安排调查；事实已足以形成候选时返回讨论完成选择，再由 Workflow 把选定方向路由到设计或作业。

现有证据是否充分影响验证安排，不自动决定开发方法。满足相应直接执行边界时，声明式数据、映射、文案、普通字段和简单标注通常直接实现并使用真实装配或消费路径核对，不写同源镜像测试。

选择调查方法前，先盘点当前上下文（包括同一会话）已有结论、证据、支持的推导和适用条件。用户的未知或“调查、求证、验证”等动作词本身不构成代理的证据缺口；只有某项新观测可能改变回答或决策时才选择调查方法。已有事实直接回答问题，或通过已确认不变量足以可靠推出答案时，复用证据并直接报告；用户明确要求新的观测、过程或产物时，把该要求作为目标处理。当前单元内可由五分钟自发调查预算回答的直接相关问题先做轻量探测；预计需要更多投入或形成独立证据目标时，返回 Workflow 建立正式调查单元。
...
| 当前事实已直接回答原问题，或通过已确认不变量足以可靠推出答案，且适用条件未变 | 复用证据并直接报告结论，不加载或运行专项调查方法 |
| 只需核对一个可直接读取的确定事实 | 直接读取权威源并报告证据，不加载专项调查方法 |

探索发现稳定症状后结束探索单元，再以系统化调试追因。调查达到足以回答原问题的证据后停止；追加调查动作前，先说明哪种结果仍可能改变当前结论或决策，没有这种结果就直接收敛。需要保留实现或长期产物时返回工作流建立开发单元。
