...
微小任务直接执行，并在作业节点内处理当前增量需要的局部职责、状态权威和激活边界。常规及以上工作单元在专项方法没有额外反馈、设计或证据收益，需求、真实调用方、契约和影响实现结构的架构决定均已明确时，直接执行；稳定概念、职责归属、生命周期、状态权威或运行接入方式仍需确定时，相关事实不足则返回工作流安排调查，事实已足以决策则返回设计。其余局部实现细节留在作业节点内处理。

现有证据是否充分影响验证安排，不自动决定开发方法。满足相应直接执行边界时，声明式数据、映射、文案、普通字段和简单标注通常直接实现并使用真实装配或消费路径核对，不写同源镜像测试。

选择调查方法前，先盘点当前上下文（包括同一会话）已有结论、证据、支持的推导和适用条件。用户的未知或“调查、求证、验证”等动作词本身不构成代理的证据缺口；只有某项新观测可能改变回答或决策时才选择调查方法。已有事实直接回答问题，或通过已确认不变量足以可靠推出答案时，复用证据并直接报告；用户明确要求新的观测、过程或产物时，把该要求作为目标处理。
