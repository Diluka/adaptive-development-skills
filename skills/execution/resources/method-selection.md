...
专项方法没有额外反馈、设计或证据收益，需求、真实调用方、契约和影响实现结构的架构决定均已明确，职责、所有权、状态权威和激活边界已经收敛时，直接执行。稳定概念、职责归属、生命周期、状态权威或运行接入方式仍需确定时，相关事实不足则返回 Workflow 安排调查，事实已足以决策则返回设计；其余局部实现细节留在作业节点内处理。

现有证据是否充分影响验证安排，不自动决定开发方法。上述架构边界已经收敛时，声明式数据、映射、文案、普通字段和简单标注通常直接实现并使用真实装配或消费路径核对，不写同源镜像测试。
...
| 当前主要目标 | 首选做法 |
|---|---|
| 需求、调用方、契约和影响实现结构的架构决定已清楚，专项方法不增加独立反馈 | 直接实现，并按 `verification-selection.md` 补最小充分证据 |
