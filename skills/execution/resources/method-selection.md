...
微小任务按 Workflow 直接执行，并在作业节点内明确当前增量需要的局部职责、状态权威和激活边界。常规及以上工作单元在专项方法没有额外反馈、设计或证据收益，需求、真实调用方、契约和影响实现结构的架构决定均已明确时，直接执行；稳定概念、职责归属、生命周期、状态权威或运行接入方式仍需确定时，相关事实不足则返回 Workflow 安排调查，事实已足以决策则返回设计。其余局部实现细节留在作业节点内处理。
