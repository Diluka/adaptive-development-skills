---
name: documentation
description: Use when 需要编写或持续维护文档——临时实施计划，README，以及 Wiki/知识库的创建、组织、来源摄取、修订或维护；也用于 Issue / 工作项、需求、设计、接口、规格、决策记录（ADR）、决策简报和交付对外文档等正式项目文档；用户或项目规范要求创建、保留、更新或版本控制长期资料时（write, organize, maintain or version-control plans, README, Wiki or formal project documentation）
---
...
文档按生命周期分两类：**临时计划**（帮助协调与降险，可丢弃）与**正式项目文档**（记录需要长期维护或共享的项目事实，进入项目约定的持久载体）。设计节点是否存在由 [adaptive-development-workflow](../adaptive-development-workflow/SKILL.md) 的 `resources/node-routing.md` 按复杂度和用途编排；进入节点后，本技能只负责文档形态、写作和维护，不承担 Workflow / Execution 的跨方法路由。
