...
- **方法内前提**：需求没有明确具体设计，需要跨业务与技术角色用可执行行为澄清共享理解；已有完整共享理解、当前工作只维护正式规格链，或只是已有独立判据的确定性零件开发时，直接使用更贴近目标的做法。
...
行为已明确且不存在共享理解缺口时，结束本方法并直接实现。不为采用 BDD 组织无助于澄清共享理解的会议、场景文件或自动化。
...
Discovery 是以具体示例为中心的收敛型对话；它不负责发散生成竞争方案或替用户决定业务取舍。需要发散设计方向时使用 [brainstorming](../brainstorming/SKILL.md)。可用 Example Mapping（规则 / 示例 / 问题卡片）或 Three Amigos（业务、开发、测试三角色）等可选技术组织对话，非必需。Discovery 的结果是共享理解和仍待确认的问题，不是越多越好的示例清单。
...
3. 内部确定性规则具有独立目标、产物和反证边界时，分开实现和验证；完成后回到当前 Automation 验证业务结果。
...
- 业务示例揭示出值得编码的稳定领域状态时，可使用 [type-driven-design](../type-driven-design/SKILL.md)。BDD 继续拥有可观察业务结果，不检查内部类型形状。
- 不自动化所有示例。按回归风险、失败信号和维护成本选择长期证据。
- 关键业务取舍尚未决定时使用 [brainstorming](../brainstorming/SKILL.md)；BDD 用示例揭示差异，不替用户决定业务政策。
- 复杂功能还需要让需求、设计、任务和实现持续可追溯时使用 [spec-driven-development](../spec-driven-development/SKILL.md)。BDD 只拥有当前行为切片的 Discovery、Formulation 与 Automation，规格链可链接其场景和结果但不重复实现。
- 文档管理不替代 Discovery、Formulation 与 Automation。
