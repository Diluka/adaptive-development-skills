...
用可丢弃的试验与会话回答未知，不提前编写正式实现，也不在同一工作单元内实现修复。

技术探索（technical spike）回答明确的可行性、性能或集成未知；探索性测试（exploratory testing）考察可运行系统的行为和风险。两者都只交付结论与证据。即使整体请求同时包含调查和实现，本单元仍受适用计划和授权边界约束。
