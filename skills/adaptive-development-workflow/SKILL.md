...
每个开发任务都要在正式实现前通过 [writing-plans](../writing-plans/SKILL.md) 写一份计划，并在交付前通过 [requesting-code-review](../requesting-code-review/SKILL.md) 接受独立子代理评审。风险决定评审深度。开发分支、TDD、实现子代理、PR/MR 和提交仍按收益、风险与授权选择。
...
是否写过代码、改过文件或执行过写操作都不是判据。临时产物必须隔离，不能混入最终差异；调查结束时检查工作区。外部写入、破坏性操作、正式环境和权限扩大仍受授权与安全边界约束。
