...
开发任务和准备正式纳入版本控制的项目变更默认完成普通 Git 交付：通过目标仓库的实际 `git remote` 判断托管平台，GitLab 创建 MR，GitHub 创建 PR，并遵循仓库自己的分支前缀、Draft/Ready、指派和其他交接约定。用户明确要求仅修改本地、只提供差异、不提交、不推送或不创建 MR/PR 时，按限制逐项收窄；非开发任务不自动产生分支、提交、推送或 MR/PR。

默认普通 Git 交付停在 MR/PR。合并、发布、部署、正式环境写入、权限修改和其他高影响外部操作只有在用户明确要求且安全边界成立时才执行。
...
7. 开发任务完成默认普通 Git 交付或用户明确收窄后的结果；非开发任务只交付其结论与证据。合并、发布、部署和其他高影响外部操作只按用户明确要求执行，并报告实际结果、限制和剩余风险。
