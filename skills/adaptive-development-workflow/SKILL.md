...
根代理以本技能分类和路由项目任务：先定交付意图与当前增量，再按目标和证据边界拆单元；每单元选一主调查/开发方法，按需叠加集成、交付和生命周期支持。它只管分类、选法和转换；新证据改变性质、判据或影响面时重走路由。用户直接调用方法时，由该技能核验适用性。
...
SDD 或 BDD 产生的实现目标另拆实现/证据单元并重新路由；计划、文档和架构调查只是支持。
...
| 目标、真实调用方和契约已经足以实施，增量局部，以上专项方法没有额外反馈或证据收益 | 直接实现；测试按实际证据缺口决定 |
| 删除死代码、适配器、导出、依赖或升级依赖等维护变更 | [maintenance-operations](../maintenance-operations/SKILL.md) |

声明式数据、映射、文案、普通字段和简单标注通常直接实现，不要求编码前证据齐备；缺少既有证据不改变主方法。解释、合并或消费它们的行为另按风险路由，不能以镜像断言替代使用方证据。测试步骤与强度按实际证据缺口决定。
...
多个方法条件同时成立时，先检查是否有独立目标、产物和反证边界。可分开就建有依赖的单元；否则选最直接解决主要不确定性或回归风险的方法，其他方法只提供技术或既有产物。
...
常规与复杂任务按“计划 → 实现 → 测试 → 评审 → 返工”编排，不另成主要方法；实现必经，其余步骤只在能补现有证据缺口时保留，复杂或高风险任务保留完整闭环。
...
3. **测试**：按真实缺口用 [choosing-tests](../choosing-tests/SKILL.md) 选择方式与强度，从需求或规格派生验收证据并验证可观察结果。
...
短小、只读、无持久写入所有权的局部步骤走 [subagent-driven-development](../subagent-driven-development/SKILL.md)；粗粒度持续写入单元走 [using-git-worktrees](../using-git-worktrees/SKILL.md) 并另配负责上下文，仍受环境能力和用户授权约束，评审者保持独立。
...
只同步本次变更直接影响的既有伴生产物。保持额外问题不变仍能完成任务时，它不属当前范围；收集最小证据并报告，不自动实施。
