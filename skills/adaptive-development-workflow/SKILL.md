...
description: Use when 开始任何项目开发、技术调查、集成或交付任务，需要按当前增量分级、按独立目标与证据边界拆分工作单元，并为调查、开发、集成与交付选择方法和生命周期支持
---
...
一个开发任务可以先包含调查单元。即使用户一开始已经授权“调查并实现”，调查方法也只交付证据；正式变更必须形成新的开发单元并重新选择开发方式。决定保留临时产物时，立即停止扩展它，明确保留、重写和删除范围后重新路由。
...
| 级别 | 可观察条件 | 最低充分流程 |
|---|---|---|
| 微小 | 目标明确、局部可逆，不改变运行行为或公共契约，不涉及依赖、生成规则、权限或产物自身的外部副作用，并能直接检查 | 直接实现、审阅差异、运行必要检查并简要总结；不落盘计划，不派最终独立评审 |
| 常规 | 有边界明确的行为或配置变化，或需要若干相关步骤，但没有重大未知或高风险 | 按协调成本决定是否落盘计划，使用覆盖受影响路径的定向验证；仅在独立视角能补充真实风险证据时评审 |
| 复杂或高风险 | 跨模块或公共契约、架构与依赖迁移、安全、身份、资金、数据、并发、破坏性或难回退操作、正式环境、显著未知，或多代理共享状态 | 使用 [writing-plans](../writing-plans/SKILL.md) 维护统一计划，按风险扩展验证，并通过 [requesting-code-review](../requesting-code-review/SKILL.md) 完成独立评审 |

从最低充分级别开始，真实影响面变化时升降级。对计划、委派、测试、评审或交付节点都要说明它能发现哪种合理错误；已有证据覆盖同一风险或节点没有实际收益时跳过。
...
- **调查单元**：消除当前系统、根因、契约、可行性、设计取舍或未知行为的不确定性，只交付事实、证据或决策输入。
- **开发单元**：交付项目需要保留、集成或发布的变更，以明确需求、真实调用方、已核验契约或已接受的调查结论为依据。

调查单元到达足以决策的证据后立即结束。若结论要求正式修复、长期测试、契约产物、规格或实现，把这些目标返回入口，重新分级并选择开发方式；不得让调查技能顺手实施。
...
直接实现不要求证据在编码前已经齐全。是否需要新增普通回归测试、静态检查、集成验证或冒烟，由 [evidence-based-testing](../evidence-based-testing/SKILL.md) 根据实现后的真实缺口决定；缺少既有证据本身不会改变单元的主要开发方式。

## 组合开发流程：计划 → 实现 → 测试 → 评审 → 返工

常规与复杂开发任务默认按“计划 → 实现 → 测试 → 评审 → 返工”的组合流程推进。这是对既有方法的标准编排，不是新的主要开发方式；每一步引用对应技能的标准流程，本入口不复制细节。
...
3. **测试**：按计划从需求或规格派生验收证据，在保留目标风险的最高真实链路验证结果，见 [evidence-based-testing](../evidence-based-testing/SKILL.md)。
...
**简化**：除实现外，每一步都可以按任务级别与证据收益省略。微小任务只实现并做必要直接检查；常规任务按需补计划、定向测试或评审；复杂或高风险任务保留完整闭环。
...
集成与交付发生在开发方法之外，可以组合，但都不能替代功能、契约或质量证据。
