...
协调者或单代理写回状态、关键证据、偏差和剩余风险，其他执行者只回报变化，由协调者核验并维护统一计划。当前 Codex 任务内的短小局部委派使用 [subagent-driven-development](../subagent-driven-development/SKILL.md)；粗粒度单元需要持续写入、独立上下文、工作树或恢复生命周期时，使用 [using-git-worktrees](../using-git-worktrees/SKILL.md) 选择由当前会话 Handoff 或复用工作树，还是在已有授权下 Fork/Create 新任务。两种方式都不让多个执行者并发编辑计划。
