...
- 不统一强制任务创建开发分支、实现子代理、独立评审、PR/MR 或提交；一旦为可能产生项目改动的工作创建或检出开发分支，且用户没有指定其他安排，按 [using-git-worktrees](../using-git-worktrees/SKILL.md) 默认绑定独立工作树。纯只读分支审查可由执行者按同一技能判断是否安全使用根路径。
