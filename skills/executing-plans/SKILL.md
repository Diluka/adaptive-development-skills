...
1. 完整阅读计划、仓库说明、工作树状态、用户最新要求和已有进展。用户明确要求直接在根工作区操作时，先通过 [using-git-worktrees](../using-git-worktrees/SKILL.md) 确认并遵循该例外；用户没有该要求时，确认当前是本任务专属关联工作树，否则先用同一技能建立隔离。开发任务没有落盘计划时，使用 [writing-plans](../writing-plans/SKILL.md) 补齐后再正式实现。
...
- 开发任务默认使用任务专属工作树；只有用户明确要求时才直接在根工作区实施。不统一强制实现子代理、TDD、PR/MR 或提交；开发任务的最终独立评审不可省略。
