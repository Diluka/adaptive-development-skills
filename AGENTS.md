...
- **结构**：`adaptive-development-workflow` 负责宏观工作流决策，`execution` 负责微观执行决策与落实，独立方法技能 / `execution/resources/` 负责选定后的方法闭环。
...
- **原则**：
  ...
  - **最小知识面**：默认入口只保留当前决策所需信息；下游方法、通用风险和生命周期细节按命中条件加载。内容详实不是保留理由。
  - **分层职责**：Workflow 只负责宏观编排；Execution 是唯一微观选法入口；独立技能与方法资源只讲选定后「怎么做」，不承担跨方法路由。
...
- **复杂或高风险任务**：选定增量本身涉及难回退或高影响操作、正式环境、安全、身份、资金、敏感数据、广泛不兼容契约迁移或显著未知。必须落盘计划并完成独立评审，按实际风险增加验证和恢复安排。

- 复杂度只由用户已选择的交付内容决定；候选方案、已排除行为和通用风险不参与分级。代理数、会话数、工作树、计划篇幅、验证数量和协作方式属于执行机制。真实跨单元协调、跨会话恢复或高风险控制可以产生落盘计划需求；仅为提速使用子代理时沿用当前单元的简短执行清单。

## 计划与协作

- 计划只承载已经完成选择的内容。影响用户可见行为、公共契约、架构边界或风险的候选先完成选择；满足 Workflow 直接作业判据时立即开工，多步骤最多维护简短执行清单。
...
- 可实施逻辑追溯到明确需求、真实调用方、已核验契约或已观察问题。用户要求发散只授权提出和比较候选；候选完成选择后进入设计、任务、验收、测试与评审。普通实现取舍在保持用户可见行为、公共契约、架构边界和风险时采用最简单方案。
- 每个调查或开发单元只选一个主要方法；交付和生命周期支持按需叠加。选定方法保留标准核心循环：Execution 资源中的 SDD 保留 Requirements/Design/Tasks/Implement/Evolve，BDD 保留 Discovery/Formulation/Automation，独立方法保留各自闭环。
...
- **技能包结构**：`adaptive-development-workflow`（唯一宏观决策入口）+ `execution`（唯一微观决策与执行入口，含按需方法 `resources/`）+ 独立方法 / 机制技能；技能↔节点多对多。
...
- 写简洁、正面的决策规则。项目产物以当前路线为主体，范围在一个权威位置由目标、穷尽的纳入项、必要约束和验收标准闭合；该范围已经完整承载用户列出的排除要求时，省略被排除对象的名称。相邻路线会改变实施、验收、授权或契约 / 兼容性判断时，只在决策依据处记录一次其与决策标准冲突的原因，决策、实施和后果继续描述当前路线。定稿时逐句检查否定标记，只保留安全 / 授权 / 隐私 / 合规红线、真实负向行为契约、影响结论的证据缺口和一处带具体原因的淘汰路线，其余内容正向改写或删除。计划、验证、评审、工作树、委派和默认交付按单元、风险、证据和授权决定。
- `adaptive-development-workflow` 负责分类、拆分、分级、节点编排与转换；`execution` 负责选法、证据与机制并执行。用户直调独立标准方法时由该方法自行核验；BDD、SDD 和类型驱动设计先由 Execution 核验资源前提。独立技能与方法资源各自维护自己的核心循环和边界。
