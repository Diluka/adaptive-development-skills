...
- **结构**：`adaptive-development-workflow` 是唯一根入口；当前事实仍有方法或证据选择缺口时，按需读取其 `resources/method-selection.md` 和证据资源。独立机制技能保留委派子代理选中后只需加载的专门约束。
...
- **技能包结构**：`adaptive-development-workflow` 是唯一根入口，`resources/method-selection.md` 与证据资源按实际缺口读取；独立机制技能承载委派子代理所需的专门约束。
...
- `verification-before-completion` 核对完成声明与最终证据，不替代过程中的验证选择；证据复用、失效与收敛见 `adaptive-development-workflow/resources/evidence.md`，按实际缺口读取。
