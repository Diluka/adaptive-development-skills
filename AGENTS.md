...
- **复杂或高风险任务**：涉及跨模块或公共契约、架构与依赖迁移、安全、身份、资金、数据、并发、破坏性或难回退操作、正式环境或显著未知。必须落盘计划并完成独立评审，按风险增加验证、专项评审和回退安排。

- 复杂度只由任务交付内容决定；代理数、会话数、工作树、计划篇幅、验证数量和协作方式属于执行机制。真实跨单元协调、跨会话恢复或高风险控制可以产生落盘计划需求；仅为提速使用子代理时沿用当前单元的简短执行清单。各节点与机制提供可说明的协调、恢复、风险控制或证据收益；远端引用变化不自动提高内容风险。
...
- 可实施逻辑追溯到明确需求、真实调用方、已核验契约、已观察问题，或通过 Execution 统一门禁的必要技术前置。用户要求发散只授权提出和比较候选；候选完成选择后进入设计、任务、验收、测试与评审。普通实现取舍在保持用户可见行为、公共契约、架构边界和风险时采用最简单方案。
