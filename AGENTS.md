...
- 文档遵循项目规范；没有规范时，临时产物放已忽略的 `.docs/`（计划在 `.docs/plans/`），正式文档放 `docs/`。
- 只有根代理可以派发子代理；子代理不得继续派生。
...
- 微小任务不落计划；常规任务仅在协调、恢复或降险有收益时落盘；复杂任务必须计划。内容、位置和阻塞前置条件遵循 `writing-plans`。
- 已授权、无关键歧义且风险受控时，计划是执行依据，不是二次批准。偏离已授权行为、范围或交付，关键歧义，高风险/难回退，正式环境或非普通 Git 外写，权限扩大或新增副作用时先确认。
- 新事实不改变目标、范围、行为、交付、授权或副作用时，更新计划并告知后继续；否则暂停受影响路径并确认。
- 一份可修正计划覆盖整个任务。根代理维护唯一计划；其他代理只读相关部分并回报，不并发修改。计划创建、更新、同步和评审处理仍属原任务，且低于需求、真实调用方、契约和新证据。

## 委派与独立评审

- 仅在边界清晰且收益高于交接成本时委派；根代理持有需求、范围、授权、写入、跨单元决策、集成和最终结论。
- 微小任务不做最终独立评审；常规任务只补定向验证未覆盖的风险；复杂任务由未参与实现、证据设计或相关差异修改者审阅完整最终差异。
- 评审者只读需求、计划、完整差异和最新证据，按严重性报告文件和行号；不得修改或继续派生。实质修改只使受影响评审失效。
...
- `description` 以 `Use when` 开头，用中英描述触发条件，不概述流程。正文以中文为主；术语规则集中在 `skills/adaptive-development-workflow/resources/terminology.md`。
