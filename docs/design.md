...
每个决策层只加载当前选择所需的信息。默认入口不保存下游方法的步骤、通用风险清单或生命周期机制；这些内容即使正确、完整，只要当前决策不需要，就留在命中条件明确的按需资源或独立技能中。
...
- **按决策点保持最小知识面**：入口只保留当前路由和执行所需信息；未命中的方法、通用风险与后续生命周期不进入默认上下文。内容详实不是保留理由，能由下游命中条件准确加载的内容从上游删除。
...
- **设计可追溯**：进入选定设计、任务、验收、测试和评审的每项可实施逻辑，都能追溯到明确需求、真实调用方、已核验契约或已观察问题。用户要求发散、深入设计或考虑更多方案时，可以提出和比较候选；影响用户可见行为、公共契约、架构边界或风险的候选在完成选择后进入下游产物。候选说明保留在讨论或专门决策材料中，实施计划以选定范围为边界。
- **自主选择最小方案**：保持用户可见行为、公共契约、架构边界和风险的普通实现取舍，由 Execution 选择最简单方案并简要说明。执行中确认某项既有设计逻辑多余，且需求、公共契约和验收结果保持一致时，方案向更小增量收敛并同步适用计划。
- **复杂度取决于选定交付内容**：按已经纳入的行为、契约、影响、未知、可逆性和副作用分级；候选方案、用户已排除的行为和通用风险不参与分级。代理数、会话数、工作树、计划篇幅、验证数量和协作方式属于执行机制。
...
- **直接进入作业**：目标、范围和验收结果明确，依赖就绪；剩余选择只是既有行为与契约内的局部实现细节。多步骤工作最多维护简短执行清单。
- **作业 → 验证**：实现完成、可观察行为就绪。
...
- **何时进入**：先盘点当前会话已有事实及其适用条件。代理仍有会改变回答或决策的真实未知时进入调查；已有证据足以回答时直接报告结论与依据。用户明确要求新的观测、过程或产物时，把它作为实际目标按其性质路由。当前动作所需的直接读取或检查留在原节点；取证本身成为独立目标时才建立调查单元。
- **怎么选方式**：针对不同问题使用合适的调查方式，按问题类型选择：

| 调查问题                                      | 调查方式                           |
| --------------------------------------------- | ---------------------------------- |
| 系统结构、运行路径、状态所有权或历史演进未知  | system-understanding（系统理解）   |
...
- **时机判据 = 任务内容 + 文档用途**：影响用户可见行为、公共契约、架构边界或风险的候选先完成选择。用户或仓库要求正式依据，任务内容复杂或高风险，或者选定结果仍需要共享的实施与验收依据时，安排设计节点；其余直接作业。
...
- **判据二：直接作业 vs 设计**
  - 目标、范围、验收和依赖均已就绪，剩余选择属于局部实现细节 → 直接作业，多步骤最多维护简短清单。
  - 选定内容复杂或高风险，或仍需形成共享的实施与验收依据 → 进入设计节点。
- **方法选择（看任务）**：
...
4. **分级**：微小 / 常规 /
   复杂（只按选定增量的行为、契约、影响、未知、可逆性和副作用）。
5. **编排工作流**：六节点（讨论 / 调查 / 设计 / 作业 / 验证 /
   评审）；动态编排（目的不明先讨论，随进展派生；明确指令 /
   已选方案跳过讨论后按级别进入后续节点）；重编排 = 返工循环 +
...
`execution` 是唯一微观决策与执行入口：正文只保留当前动作所需的通用执行循环、范围反馈和资源路由；
