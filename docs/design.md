...
| 层级           | 角色                   | 内容                                                     |
| -------------- | ---------------------- | -------------------------------------------------------- |
| **① 宏观决策** | 任务入口的总路由       | 判断任务侧重点、编排工作流、处理重编排                   |
| **② 微观决策** | 每个工作节点的局部判断 | 各节点该怎么做、何时做、做到什么程度                     |
| **③ 方式方法** | 落地执行的具体方法论   | SDD / BDD / TDD / 头脑风暴 / 测试 / 调查 / 验证 / 评审等 |
...
为不增加独立技能，**两个决策层（宏观 +
微观）由入口技能一个说清**：微观决策内容作为入口技能的 `resources/`
资源文本而非技能正文，由对应工作代理按需深入阅读。

#### 决策（入口）

| 技能                            | 定位                                                                                                                                                            |
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `adaptive-development-workflow` | 宏观决策（宽泛说明）：定义节点类型、编排工作流、处理重编排、进入节点推荐技能集；`resources/` = 微观决策资源（各节点决策规则，按需读取，仅对应工作代理深入阅读） |

#### 方法 · 执行与机制

| 技能                             | 定位                                                                 | 来源                              |
| -------------------------------- | -------------------------------------------------------------------- | --------------------------------- |
| `execution`                      | 在工作节点上执行工作单元（落实具体执行；有计划引用、无计划直接执行） | 原 `executing-plans` 改名泛化     |
...
| `verification-before-completion` | 完成声明前核对证据                                                   | 保留（与验证节点资源衔接）        |
| `delivery`                       | 交付节奏（主干 / 持续交付 / 渐进暴露）                               | 保留（内部重组为决策 + 方法资源） |
| `evidence-management`            | 证据充分性 / 复用 / 收敛                                             | 保留（剥离方法路由表）            |
...
| 测试 | `property-based-testing`、`consumer-driven-contract-testing`、`baseline-and-eval-testing`                 |
| 调查 | `system-understanding`、`contract-verification`、`systematic-debugging`、`unknown-exploration`            |

#### 拆分并入

- `choosing-tests` 拆分并入入口 `resources/`（验证方式选择）。
...
- **分层职责**：宏观决策技能负责流程编排；方法技能只讲「怎么做」，不承担流程编排——方法技能可声明自己的方法边界（何时停止返回宏观重新编排），但不描述宏观如何安排下一步。
- **推荐 /
  加载策略**：简单方法（内容简短）可在节点推荐中直接提及，用不用皆可；复杂方法（内容详尽）须先经决策确定适用，再**按需读取**其说明——不在无关工作中加载复杂方法全文，以免污染上下文。技能的触发描述应足以让代理判断是否需要读取全文。
- **宽泛说明 vs
  具体执行**：入口技能承载**宽泛说明**（节点定义、编排、推荐技能集）——所有代理可了解；**具体执行**由
  `execution`
  等执行类技能落实对应定义——需要干活的代理按需研究其细节，无关代理不加载具体执行内容。

### 5.3 节点 → 推荐技能映射

进入某个工作节点时，入口技能推荐适用的技能集。每个技能标注：类别（判据 / 方法 /
机制）× 主辅（主 = 默认首选，通常用一种；辅 = 拆分 / 辅助）×
加载方式（复杂按需）。**判据统一放入口
`resources/`（各节点决策规则）**，正文只做推荐表。

| 节点 | 判据（入口 `resources/`） | 推荐技能（方法 / 机制）                                                                                                                                                                                                                                                              |
| ---- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 讨论 | 何时讨论                  | `brainstorming`（方法 · 主）                                                                                                                                                                                                                                                         |
| 调查 | 按问题类型选方式          | system-understanding / contract-verification / systematic-debugging / unknown-exploration（方法 · 主）                                                                                                                                                                               |
| 设计 | 何时写什么文档            | `documentation`（方法 · 主）                                                                                                                                                                                                                                                         |
| 作业 | 方法选择（开发方式选一）  | `execution`（方法 · 主）；SDD / BDD / TDD / 维护（方法 · 主，默认一种、可拆分）；`type-driven-design`（**设计方式** · 叠加推荐 · 与开发方式正交）；属性 / 契约 / 基线（方法 · 辅）；`documentation`（方法 · 辅）；`using-git-worktrees` / `agent-and-parallel-dispatch`（机制 · 辅） |
| 验证 | 验证方式选择              | `execution`（方法 · 主）；属性 / 契约 / 基线（方法 · 主/辅）；`verification-before-completion`、`evidence-management`（方法 · 辅）；`agent-and-parallel-dispatch`（机制 · 辅）                                                                                                       |
| 评审 | 何时需要评审              | `requesting-code-review` / `receiving-code-review`（方法 · 主）；`agent-and-parallel-dispatch`（机制 · 评审必须分离）；`using-git-worktrees`（机制 · 辅）                                                                                                                            |
...
### 5.4 入口技能设计（adaptive-development-workflow）

入口技能是唯一承载决策的技能：正文 =
宏观编排（宽泛说明，所有代理可了解）；`resources/` =
微观决策资源（各节点判据，对应代理按需读取）。

**正文结构（宽泛说明）**：

1. **入口职责**：定义工作节点类型、编排工作流、处理重编排、进入节点推荐适用技能集；微观判据在
   `resources/` 按需读取，正文不展开。
2. **识别任务侧重点**：讨论 / 调查 / 作业 /
...
6. **进入节点推荐技能集**：按 §5.3 映射推荐（判据 / 方法 / 机制 × 主辅 ×
   加载）；判据在 `resources/` 对应资源。
7. **叠加交付与生命周期支持**：`delivery`、`finishing-a-development-branch`、`verification-before-completion`
   等按需叠加。
8. **执行与转换**：按依赖执行；新证据改变任务性质、判据或影响面时重新编排。
9. **安全边界**：不写正式环境数据源；保留他人改动；不为测试加生产开关 / 包装。

**`resources/` 结构**（按需读取，对应代理深入阅读）：

- `node-decisions/`：各节点决策规则（讨论 / 调查 / 设计 / 作业 / 验证 /
  评审）——整理自 §4.2；
- `verification-selection.md`：验证方式选择（动态 / 静态 / 审视 + 组合，原
  `choosing-tests` 并入）；
